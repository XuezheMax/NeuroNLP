...
from .nlinalg import theano_logsumexp, logabsdet_safe
...
def tree_crf_loss(energies, heads, types, masks):
    """
    compute minus log likelihood of tree crf as tree crf loss.
    :param energies: Theano 4D tensor
    ...
    I = T.eye(length, length, 0)
    potentials = T.exp(T.switch(I.dimshuffle('x', 0, 1, 'x'), -np.inf, energies))
    ...
    partitions, _ = theano.scan(fn=lambda laps, len: logabsdet_safe(laps[1:len, 1:len]), outputs_info=None,
                                sequences=[L, lengths])
