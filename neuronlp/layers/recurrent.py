...
class TARULayer(MergeLayer):
    """
    TARULayer based on lasagne.layer.GRULayer, with recurrent dropout options.
    ...
    def __init__(self, incoming, num_units, num_time_units=None, max_length=100,
        ...
        # Initialize parent layer
        super(TARULayer, self).__init__(incomings, **kwargs)
        ...
        self.P_time = self.add_param(P_time, (max_length, num_time_units), name="P_time", regularizable=False)
    ...
    def get_output_for(self, inputs, deterministic=False, **kwargs):
        ...
        def step(p_n, input_n, hid_previous, *args):
            ...
            # compute time_state
            time_state = (1 - time_updategate) * p_n + time_updategate * time_update

            time_input = T.dot(time_state, W_state_stacked)
        ...
        def step_masked(p_n, input_n, mask_n, hid_previous, *args):
            hid = step(p_n, input_n, hid_previous, *args)
...
class MAXRULayer(MergeLayer):
    ...
    def get_output_for(self, inputs, deterministic=False, **kwargs):
        ...
        if not deterministic and self.p:
            one = T.constant(1)
            retain_prob = one - self.p
            time_init = (time_init / retain_prob) * dropout_mask_time
            hid_init = (hid_init / retain_prob) * dropout_mask_hidden
